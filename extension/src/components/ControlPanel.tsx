...
import { Copy, Trash2, Volume2, VolumeX, List, FileText } from 'lucide-react';
...
// --- Simple log view helpers ---

interface SimpleLogEntry {
    time: string;
    type: 'fail' | 'success' | 'session-end';
    attemptNum: number;
    maxRetries: number;
    percent?: string;
    reason?: string;
...
function extractSimpleLogs(logs: string[]): SimpleLogEntry[] {
    ...
    for (const line of logs) {
        ...
        // Track video progress from "Progress: X/Y videos" lines
        const progressMatch = line.match(/Progress: (\d+)\/(\d+) videos/);
        if (progressMatch) {
            videoGoal = parseInt(progressMatch[2], 10);
            continue;
        }

        // Track video goal from "Video goal reached" lines
        const goalReachedMatch = line.match(/Video goal reached.*?(\d+)\/(\d+)/);
        if (goalReachedMatch) {
            videoGoal = parseInt(goalReachedMatch[2], 10);
            continue;
        }

        // Failure: "Failed at X% — Security Layer ..."
        const failMatch = line.match(/Failed at (\S+) — (.+)$/);
        if (failMatch) {
            const percent = failMatch[1];
            const rawReason = failMatch[2].trim();
            // Strip "Security Layer N:" prefix — show only the actual reason
            const reason = rawReason.replace(/^Security [Ll]ayer \d+:\s*/i, '');
            let colorClass = 'text-yellow-500';
            if (rawReason.includes('Layer 3')) {
                colorClass = 'text-red-400';
            } else if (rawReason.includes('Layer 2')) {
                colorClass = 'text-orange-400';
            } else if (rawReason.includes('Layer 1')) {
        ...
        if (line.includes('SUCCESS') && line.includes('Success detected')) {
            successCount++;
            entries.push({
                time,
                type: 'success',
        ...
        // Session end: rate limit
        if (line.includes('rate limit') || line.includes('Rate limit')) {
            entries.push({
                time,
                type: 'session-end',
                attemptNum: currentAttempt,
        ...
        // Session end: max retries reached
        if (line.includes('Session ended') && line.includes('Max retries')) {
            entries.push({
                time,
                type: 'session-end',
                attemptNum: currentAttempt,
        ...
        // Session end: goal reached
        if (line.includes('Session ended') && line.includes('Goal reached')) {
            entries.push({
                time,
                type: 'session-end',
                attemptNum: currentAttempt,
        ...
        if (line.includes('Session ended') && line.includes('Cancelled')) {
            entries.push({
                time,
                type: 'session-end',
                attemptNum: currentAttempt,
...
function formatSimpleLogLine(entry: SimpleLogEntry): string {
    if (entry.type === 'fail') {
        return `${entry.time} — ${entry.attemptNum}/${entry.maxRetries} — Failed at ${entry.percent} — ${entry.reason}`;
    }
    if (entry.type === 'success') {
        return `${entry.time} — ${entry.attemptNum}/${entry.maxRetries} — Success ${entry.videoNum}/${entry.videoGoal}`;
    }
...
export const ControlPanel: React.FC<ControlPanelProps> = ({
    ...
    const { data: uiStorage, save: saveUI } = useStorage();
    const simpleLogView = uiStorage.simpleLogView;
    const simpleLogs = React.useMemo(() => extractSimpleLogs(logs), [logs]);
                        ...
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button
                                    ...
                                    size="icon"
                                    className="h-8 w-8"
                                    onClick={() => saveUI('simpleLogView', !simpleLogView)}
                                    aria-label={simpleLogView ? 'Show detailed logs' : 'Show simple logs'}
                                >
                                    {simpleLogView ? (
                            ...
                            <TooltipContent>{simpleLogView ? 'Detailed log' : 'Simple log'}</TooltipContent>
                        </Tooltip>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button
                                    ...
                                    onClick={() => {
                                        ...
                                        try {
                                            const text = simpleLogView
                                                ? simpleLogs.map(formatSimpleLogLine).join('\n')
                                                : logs.join('\n');
                                            if (!text) return;
                    ...
                    {simpleLogView ? (
                        simpleLogs.length > 0 ? (
                            <ul className="space-y-1">
                                {simpleLogs.map((entry, i) => (
                                    <li key={i} className={entry.colorClass}>
                                        {formatSimpleLogLine(entry)}
                                    </li>
                                ))}
                            </ul>
                        ) : (
                            <p className="text-muted-foreground">No events yet for this session.</p>
                        )
                    ) : logs && logs.length ? (
                        <ul className="space-y-1">
                            {logs.map((line, i) => {
