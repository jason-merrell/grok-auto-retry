      ...
      w.__grok_test = {
        activateSession(postId?: string) {
          try {
            ...
            const id = postId || w.__grok_activePostId || '__unknown__';
        ...
        getSessionSnapshot(postId?: string) {
          try {
            const id = postId || w.__grok_activePostId || w.__grok_forcedPostId || '__unknown__';
            const key = `grokRetrySession_${id}`;
            const stored = w.sessionStorage.getItem(key);
        ...
        startSession(postId?: string) {
          try {
            ...
            if (postId) w.__grok_attempts[postId] = now; else w.__grok_attempts['__unknown__'] = now;
        ...
        getLastAttemptTime() { return (w.__grok_attempts && w.__grok_attempts['__unknown__']) || 0; },
      };
    }
  } catch {}
}
